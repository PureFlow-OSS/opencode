...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "ProviderModelNotFoundError")) {
    const data = isRecord(input) && isRecord(input.data) ? input.data : undefined
  ...
  if (NamedError.hasName(input, "ProviderInitError")) {
    const data = isRecord(input) && isRecord(input.data) ? stringField(input.data, "providerID") : undefined
    return `Failed to initialize provider "${data}". Check credentials and configuration.`
  }
