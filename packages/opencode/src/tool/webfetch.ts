        ...
        Effect.gen(function* () {
          ...
          if (process.platform === "win32" && proxy) {
            const script = [
              ...
              "$response = $null",
              "for ($i = 0; $i -lt 2; $i++) {",
              `  try { $response = Invoke-WebRequest -UseBasicParsing -MaximumRedirection 10 -Uri '${url}' -Headers $headers -Proxy '${proxy}' -ProxyUseDefaultCredentials -TimeoutSec ${Math.max(1, Math.ceil(timeout / 1000))}; break }`,
