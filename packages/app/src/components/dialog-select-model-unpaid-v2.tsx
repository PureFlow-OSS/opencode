...
import { isModelProviderVisible, useProviders } from "@/hooks/use-providers"
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const freeModels = createMemo(() => model.list().filter((item) => isFree(item) && isModelProviderVisible(item.provider.id)))
              ...
              <div class="grid w-full grid-cols-1 gap-y-1.5 gap-x-2 sm:grid-cols-2">
                <For
                  each={[...providers.popular()]
                    .filter((provider) => featuredProviders.includes(provider.id) && isModelProviderVisible(provider.id))
