    ...
    const ensure = (key: ServerConnection.Key) => {
      ...
      const root = createRoot(
        (dispose) => ({
          key,
          dispose,
          state: createServerPermissionState({ sdk: ctx.sdk, sync: ctx.sync, settings }),
        }),
        owner ?? undefined,
      )
...
function createServerPermissionState(input: { sdk: ServerSDK; sync: ServerSync; settings: ReturnType<typeof useSettings> }) {
  ...
  function shouldAutoRespond(permission: PermissionRequest, directory?: string) {
    if (input.settings.permissions.autoApprove()) return true
  ...
  createEffect(() => {
    if (!input.settings.permissions.autoApprove()) return
    input.sdk.client.permission
      ...
      .then((result) => {
        ...
        for (const permission of result.data ?? []) {
          ...
          void respondPending(permission)
        }
      })
      .catch(() => undefined)
  })
